...
void PD_GM_MapManager::InstantiateDynamicMap() {
	...
	PD_MG_LogicPosition logicPosition = SGI->getPlayerManager()->GetCharacterByIndex(0)->GetCurrentLogicalPosition();
	PD_MG_LogicPosition logicPositionInst = PD_MG_LogicPosition(logicPosition.GetX() + 1, logicPosition.GetY() + 1);

	charac = instantiator->InstantiateOrcMelee(logicPositionInst);
	...
	logicCha->SetTypeCharacter(ECharacterType::OrcMelee);
