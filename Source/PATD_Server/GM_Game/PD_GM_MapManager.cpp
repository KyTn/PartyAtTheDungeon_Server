...
void PD_GM_MapManager::InstantiateDynamicMap() {
	...
	{
		PD_MG_LogicPosition spwpnt = spawn[rand() % (spawn.Num() + 1)];


		UE_LOG(LogTemp, Warning, TEXT("PD_GM_MapManager::InstantiateDynamicMap - Player %d is at (%d,%d)"), i, spwpnt.GetX(), spwpnt.GetY());

		_GAMEMANAGER->playersManager->GetDataPlayers()[i]->logic_Character->SetCurrentLogicalPosition(spwpnt);
		_GAMEMANAGER->playersManager->GetDataPlayers()[i]->logic_Character->SetCharacterBP(instantiator->InstantiatePlayer(spwpnt));
